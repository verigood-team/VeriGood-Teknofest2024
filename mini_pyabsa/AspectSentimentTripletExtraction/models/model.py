...
class EMCGCN(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.from_checkpoint is not None:
            ...
            max_post_size, max_deprel_size, max_postag_size, max_synpost_size = get_max_values(config, config.from_checkpoint)
        else:

            max_post_size, max_deprel_size, max_postag_size, max_synpost_size = (config.get("post_size"),
                                                                                config.get("deprel_size"),
                                                                                config.get("postag_size"),
                                                                                config.get("synpost_size"))
        
        self.config.post_size = max_post_size
        self.config.deprel_size = max_deprel_size
        self.config.postag_size = max_postag_size
        self.config.synpost_size = max_synpost_size

        # Embedding layers
        self.post_emb = torch.nn.Embedding(
            max_post_size, config.output_dim, padding_idx=0
        )
        self.deprel_emb = torch.nn.Embedding(
            max_deprel_size, config.output_dim, padding_idx=0
        )
        self.postag_emb = torch.nn.Embedding(
            max_postag_size, config.output_dim, padding_idx=0
        )
        self.synpost_emb = torch.nn.Embedding(
            max_synpost_size, config.output_dim, padding_idx=0
        )
...
def get_max_values(config, from_checkpoint):

    if from_checkpoint:
        config_path = find_files(from_checkpoint, ".args.txt")

        all_values = []
        if not config_path:
            return all_values

        with open(config_path[0], 'r') as file:
            for line in file:
                if line.startswith("post_size"):
                    parts = line.split(':')
                    if len(parts) == 2:
                        all_values.append(parts[1].strip())
                
                elif line.startswith("deprel_size"):
                    parts = line.split(':')
                    if len(parts) == 2:
                        all_values.append(parts[1].strip())

                elif line.startswith("postag_size"):
                    parts = line.split(':')
                    if len(parts) == 2:
                        all_values.append(parts[1].strip())
                
                elif line.startswith("synpost_size"):
                    parts = line.split(':')
                    if len(parts) == 2:
                        all_values.append(parts[1].strip())

        max_post_size = max(config.get("post_size"), int(all_values[0]))
        max_deprel_size = max(config.get("deprel_size"), int(all_values[1]))
        max_postag_size = max(config.get("postag_size"), int(all_values[2]))
        max_synpost_size = max(config.get("synpost_size"), int(all_values[3]))

        return max_post_size, max_deprel_size, max_postag_size, max_synpost_size