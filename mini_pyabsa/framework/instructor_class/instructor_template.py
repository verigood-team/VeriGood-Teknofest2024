...
from mini_pyabsa.framework.sampler_class.imblanced_sampler import ImbalancedDatasetSampler
from mini_pyabsa.utils.pyabsa_utils import print_args, fprint
...
class BaseTrainingInstructor:
    ...
    def load_cache_dataset(self, **kwargs):
        ...
        cache_path = self.config.load_cache_path
            ...
            if from_checkpoint_path:
                ...
                if state_dict_path:
                    if (
                        ...
                        load_state_dict_manually(self.model, state_dict_path[0], self.config.device)
                    else:
                        load_state_dict_manually(self.model, state_dict_path[0], self.config.device)
...
def get_resume_checkpoint(config):
    from mini_pyabsa.framework.checkpoint_class.checkpoint_template import CheckpointManager
...
def load_state_dict_manually(model, state_dict_path, device):

    model_state_dict = model.state_dict()
    checkpoint_state_dict = torch.load(state_dict_path, map_location=device)
    ...
    for name, param in model_state_dict.items():
        ...
        if name in checkpoint_state_dict:
            if param.size() == checkpoint_state_dict[name].size():
                ...
                count+=1
                param_shape = model_state_dict[name].size()
                if len(param_shape) > 1:
                    updated_state_dict[name] = torch.randn(param_shape).to(device)
                else:
                    updated_state_dict[name] = torch.randint(0, 10, param_shape).to(device)
        
        else:
            print(f"Parameter {name} not found in the model. Skipping...")
